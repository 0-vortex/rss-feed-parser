...
const run = async () => {
  ...
  const { data: runner } = await octokit.rest.users.getAuthenticated();
  warning("%O", runner);
  ...
  for (const user of users) {
    if (typeof parsedCache[user.login] === "undefined") {
      ...
      if (parsedCache[user.login].offsetHours >= offsetHours) {
        parseUsers.push(user);
        parsed = true;
      }
      ...
      fn(
        `${user.login} has been checked ${
          parsedCache[user.login].offsetHours
        } hours ago, ${parsed ? "adding to the queue" : "skipping"}`,
      );
    }
  ...
  await (async () => {
    ...
    const { error: err, count } = await supabase.from("events").upsert(insertEvents, {
      // returning: "minimal",
      count: "exact",
      onConflict: "id",
  ...
  await (async () => {
    ...
    const { error: err, count } = await supabase.from("users").upsert(insertUsers, {
      // returning: "minimal",
      count: "exact",
      onConflict: "id",
      ignoreDuplicates: true,
  ...
  await (async () => {
    ...
    const { error: err, count } = await supabase.from("repos").upsert(insertRepos, {
      // returning: "minimal",
      count: "exact",
      onConflict: "id",
      ignoreDuplicates: true,
